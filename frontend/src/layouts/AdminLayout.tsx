...
import { LayoutDashboard, FileText, Users, LogOut, Upload, Menu, X, Search, Bell, ChevronRight } from 'lucide-react';
...
const AdminLayout = () => {
  ...
  return <div className="min-h-screen bg-transparent flex">
      ...
      duration: 0.3,
      ease: "easeInOut"
    }} className={cn("fixed inset-y-0 right-0 z-30 transform transition-transform duration-200 lg:translate-x-0 lg:static flex flex-col", isMobileMenuOpen ? "translate-x-0" : "translate-x-full lg:translate-x-0")} data-state={isMenuCollapsed ? "collapsed" : "expanded"}>
        <div className="p-4 flex items-center justify-between mb-2">
          <div className={cn("flex items-center", isMenuCollapsed ? "justify-center w-full" : "")}>
            <div id="logo-wrapper">
              <img alt="Darsni" src="/lovable-uploads/58c2f9f8-a86e-458c-86b3-590e2530f8df.png" className="logo-full object-scale-down" />
              <img className="logo-mini" src="/assets/images/darsni-icon.svg" alt="" />
            </div>
          </div>
          
          <button onClick={toggleMenu} className={cn("p-2 rounded-lg hover:bg-white/5 text-gray-400 hover:text-white transition-colors", isMenuCollapsed ? "rotate-180" : "")}>
            <ChevronRight className="h-5 w-5" />
          </button>
        </div>
        
        <div className="p-4 border-b border-white/10">
          <div className={cn("flex", isMenuCollapsed ? "flex-col items-center" : "flex-col items-center")}>
          ...
          <nav className="space-y-2">
            ...
            <AnimatePresence initial={false}>
              ...
              duration: 0.2
                  ...
                  <Link to="/admin/courses/upload" className="btn-primary mt-6 flex items-center gap-3 py-3 px-4 rounded-xl w-full justify-center hover:shadow-lg hover:shadow-[#FF4800]/20">
        ...
        <div className="p-4 mt-auto border-t border-white/10">
          <button onClick={logout} className={cn("flex items-center gap-3 text-gray-400 hover:text-white transition-all p-3 rounded-xl hover:bg-white/5", isMenuCollapsed ? "justify-center w-10 h-10 mx-auto" : "w-full")}>
          ...
          <div className="py-3 px-4 flex justify-between items-center">
            <div className="flex items-center">
              ...
              <div className="relative mx-2 hidden md:block">
                ...
                <input type="search" className="bg-white/5 border border-white/10 text-white text-sm rounded-xl block w-80 pr-10 p-2.5 placeholder-gray-500 focus:ring-1 focus:ring-[#FF4800] focus:border-[#FF4800]" placeholder="ابحث..." />
              </div>
            </div>
