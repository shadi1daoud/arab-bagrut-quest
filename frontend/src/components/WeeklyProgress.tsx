...
const WeeklyProgress: React.FC<WeeklyProgressProps> = ({
  ...
  const bestDay = weeklyData.reduce((best, current) => current.xp > best.xp ? current : best, weeklyData[0] || {
    day: 'الأحد',
    xp: 0
  });
  ...
  const getMotivationalMessage = () => {
    if (progressPercentage >= 100) return {
      text: "ممتاز! لقد حققت هدفك الأسبوعي! 🎉",
      color: "text-green-400"
    };
    if (progressPercentage >= 75) return {
      text: "أداء رائع! أنت قريب من تحقيق هدفك",
      color: "text-[#FF4800]"
    };
    if (progressPercentage >= 50) return {
      text: "استمر! أنت في منتصف الطريق",
      color: "text-yellow-400"
    };
    if (progressPercentage >= 25) return {
      text: "بداية جيدة، واصل التقدم",
      color: "text-blue-400"
    };
    return {
      text: "ابدأ رحلتك التعليمية اليوم!",
      color: "text-gray-400"
    };
  };
  ...
  return <div className="space-y-4">
      ...
      <div className="text-center">
        ...
        <div className="relative mb-4">
          <svg width="120" height="120" viewBox="0 0 120 120" className="mx-auto">
            ...
            <circle cx="60" cy="60" r="52" fill="none" stroke="#FF4800" strokeWidth="8" strokeLinecap="round" strokeDasharray="326.7" strokeDashoffset={326.7 - 326.7 * progressPercentage / 100} className="transition-all duration-1000 ease-out" style={{
          ...
          {/* Achievement Badge */}
          {progressPercentage >= 100 && <div className="absolute top-0 right-0 bg-yellow-500 rounded-full p-1.5 animate-pulse">
        ...
        <div className="mb-3">
          ...
          <p className="text-xs font-noto mt-1" style={{
          color: motivation.color.replace('text-', '')
        }}>
            {motivation.text}
          </p>
        </div>
      ...
      <div className="bg-[rgba(255,255,255,0.03)] rounded-xl p-4">
        ...
        <div className="flex items-end justify-between gap-2 h-16 mb-3">
          ...
          return <div key={day.day} className="flex-1 flex flex-col items-center">
                <div className={`w-full rounded-lg transition-all duration-700 relative group cursor-pointer ${day.xp > 0 ? isBestDay ? 'bg-gradient-to-t from-[#FF4800] to-[#FF6B35] shadow-lg shadow-[#FF4800]/30' : 'bg-[#FF4800]' : 'bg-gray-700 hover:bg-gray-600'} ${isToday ? 'ring-2 ring-white/30' : ''}`} style={{
              height: `${height}%`
            }}>
                  {isBestDay && <Star className="h-3 w-3 text-white absolute top-1 left-1/2 transform -translate-x-1/2" fill="white" />}
                ...
                <span className={`text-[10px] mt-2 font-noto ${isToday ? 'text-white font-bold' : 'text-gray-400'}`}>
                  {day.day.slice(0, 2)}
                </span>
              </div>;
        })}
      ...
      {/* Achievement Badges */}
      {(progressPercentage >= 25 || streak >= 3 || activeDays >= 5) && <div className="flex flex-wrap gap-2 justify-center">
          {progressPercentage >= 25 && <div className="flex items-center gap-1 py-1 px-2 bg-[#FF4800]/10 rounded-full text-[10px] text-[#FF4800] border border-[#FF4800]/20 font-noto">
              <Target className="h-2.5 w-2.5" />
              {Math.round(progressPercentage)}% مُكمل
            </div>}
          
          {streak >= 3 && <div className="flex items-center gap-1 py-1 px-2 bg-[#FF4800]/10 rounded-full text-[10px] text-[#FF4800] border border-[#FF4800]/20 font-noto">
              <Flame className="h-2.5 w-2.5" />
              نار متقدة
            </div>}
          
          {activeDays >= 5 && <div className="flex items-center gap-1 py-1 px-2 bg-[#FF4800]/10 rounded-full text-[10px] text-[#FF4800] border border-[#FF4800]/20 font-noto">
