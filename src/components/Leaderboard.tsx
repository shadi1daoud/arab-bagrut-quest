    ...
    <div className="w-[330px] h-[220px]">
      <div className="flex justify-between items-center mb-4">
        <h3 className="text-sm font-bold text-white font-changa flex items-center gap-2">
          <Trophy className="h-4 w-4 text-[#8E6DFF]" strokeWidth={2} />
          المتصدرون
        </h3>
        ...
        <div className="flex bg-[rgba(255,255,255,0.03)] rounded-lg p-0.5 text-[10px]">
          <button 
            ...
            className={`px-2 py-0.5 rounded-md ${filter === 'week' ? 'bg-[#8E6DFF] text-[#0C0E1A]' : 'text-gray-400'}`}
          >
            أسبوعي
          </button>
          <button 
            onClick={() => onFilterChange('month')}
            className={`px-2 py-0.5 rounded-md ${filter === 'month' ? 'bg-[#8E6DFF] text-[#0C0E1A]' : 'text-gray-400'}`}
          >
            شهري
          </button>
        </div>
        ...
        {data.slice(0, 3).map((user, index) => (
          ...
          >
            <div className="h-6 w-6 flex items-center justify-center">
              {index === 0 ? (
                <div className="h-6 w-6 rounded-full bg-yellow-500 flex items-center justify-center text-black text-xs font-bold animate-pulse">1</div>
              ) : index === 1 ? (
